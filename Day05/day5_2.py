...
def printTopCrates(stacks:dict[int, list]):
    ...
    print(f"The crates at the top of each stack are: {''.join(topCrates)}")
...
# Modified for part 2 - Cratemover 9001: Moves an entire stack at once
def modifyStack(totalCrates, initStack, destStack, stacks:dict):
    
    # Removing totalCrates from the stack at once and placing it into the next stack
    movedStack = [stacks[initStack].pop(-1*idx) for idx in range(totalCrates, 0, -1)]

    stacks[destStack] += movedStack
